#!/usr/bin/env python3
"""
PolyBMiCoB v3 - Clean Arb MM Bot (Event-Driven)

Based on v2 architecture with simplified MM logic:
  - NO rescue completion above $1.00
  - NO last-resort completion
  - Strict pair_cost < $1.00 for completion
  - Clean arb or cancel (no salvage mode)
...
Usage:
  ...
  workon polybmicob
  python scripts/btc_bot_v3.py [--dry-run]
"""
...
def _complete_mm_pair(slug: str) -> None:
    ...
    v3 CLEAN ARB: strict pair_cost < $1.00 only. No rescue, no retry loop.
    Single orderbook check - if affordable, buy. If not, leave as directional.
    """
    ...
    for trade in mm_trades:
        ...
        # v3: Single orderbook check - no retry loop, no rescue above $1.00
        try:
            book_resp = httpx.get(
                f"{CLOB_HOST}/book",
            ...
            if not asks:
                log.info("  PAIR SKIP: %s no asks available for %s", slug, opp_dir)
                continue
...
def _handle_mm_only(slug: str, slot_ts: int) -> None:
    ...
    for mkt in markets:
        ...
        # v3: no time-to-start filter, no timeout capping
        # Simply bid on any market found, use full MAKER_TIMEOUT_SEC
...
def _handle_ending(slug: str, slot_ts: int) -> None:
    ...
    v3: NO last-resort MM completion. Partials stay as directional bets.
    """
    # Late-game hedge
    if HEDGE_ENABLED:
        try:
...
def main() -> None:
    ...
    log.info("=" * 60)
    log.info("PolyBMiCoB v3 - Clean Arb MM Bot (Event-Driven)")
    ...
    log.info("MM Strategy: CLEAN ARB (no rescue >$1.00, no last-resort, strict pair_cost<$1.00)")
    log.info(
